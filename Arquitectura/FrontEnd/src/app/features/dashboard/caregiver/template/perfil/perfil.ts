...
@Component({
    selector: 'app-caregiver-perfil',
    standalone: true,
    imports: [CommonModule, FormsModule, CaregiverMenu],
    templateUrl: './perfil.html',
    styleUrls: ['./perfil.css']
})
export class CaregiverPerfil implements OnInit {
    ...
    perfilEditado: PerfilAcompanante | null = null;
        ...
        try {
            ...
            if (storedUser) {
                try {
                    ...
                    this.userRol = userData.rol || 'Acompanante';
                } catch (e) {
            ...
            if (!this.userEmail) {
                ...
                if (user) {
                    ...
                    this.caregiverFullName = user.displayName || 'Acompanante';
                }
            }
    ...
    private async cargarPerfil() {
        try {
            ...
            const userData = await firstValueFrom(
                this.usersService.getUsuarioById(this.caregiverId)
            );

            if (userData) {
                this.perfil = {
                    idusuario: userData.idusuario || this.caregiverId || 0,
                    nombre: userData.nombre || this.caregiverName,
                    ...
                    rol: userData.rol || 'Acompanante'
                };

                if (this.perfil.nombre && this.perfil.apPaterno) {
                    this.caregiverFullName = `${this.perfil.nombre} ${this.perfil.apPaterno} ${this.perfil.apMaterno || ''}`.trim();
                }
    ...
    private async cargarPerfilPorEmail() {
        try {
            ...
            if (Array.isArray(allUsers) && allUsers.length > 0) {
                ...
                if (foundUser) {
                    this.caregiverId = foundUser.idusuario || foundUser.id || null;
                    if (this.caregiverId) {
                        await this.cargarPerfil();
                    }
                }
